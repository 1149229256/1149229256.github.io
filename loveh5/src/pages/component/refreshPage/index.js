...
export default class LgRefreshPage extends Component {
  constructor(props) {
    super(props)
  ...
  render () {
    ...
    return (
      <View className={`dragUpdataPage ${this.props.className || ""}`} style={style}>
          {
            this.props.onDown ? 
            (
              <View className='downDragBox' style={downDragStyle}>
                  <AtActivityIndicator></AtActivityIndicator>
                  <Text className={`downText ${this.state.dargState == 1 ? "downText_cho" : ""}`}>{this.state.downText}</Text>
              </View>) : null
          }
          <ScrollView
              style={dargStyle}
          ...
          </ScrollView>
          {
            ...
            (
              <View className='upDragBox' style={upDragStyle}>
                  <AtActivityIndicator></AtActivityIndicator>
                  <Text className={`downText ${this.state.dargState == -1 ? "downText_cho" : ""}`}>{this.state.pullText}</Text>
              </View>
            ) : null
          }
      </View>
